                    ...
                    <div className="card bg-base-100 shadow-xl border border-base-300 flex flex-col h-full">
                        <div className="card-body p-2.5 lg:p-4 flex flex-col h-full">
                            ...
                            <div className="flex items-center justify-between mb-2 lg:mb-3">
                                <h2 className="card-title text-primary text-sm lg:text-lg">
                                    Code Editor
                                </h2>
                ...
                <div className="flex flex-col min-h-0 flex-1 lg:h-full overflow-y-auto space-y-3 lg:space-y-4">
                    {error && (
                        <div className="alert alert-error shadow-lg text-xs lg:text-sm">
                            <svg xmlns="http://www.w3.org/2000/svg" className="stroke-current shrink-0 h-5 w-5 lg:h-6 lg:w-6" fill="none" viewBox="0 0 24 24">
                    ...
                    {hasResults && (
                        <div className="space-y-3 lg:space-y-4">
                            {['Scanner', 'Fixer', 'Validator'].map(agent =>
                                agentResults[agent] && (
                                    <AgentCard key={agent} agent={agent} result={agentResults[agent]} />
                                )
                            )}
                        </div>
                    )}
                        ...
                        <div className="card bg-base-200 shadow-xl flex-1 flex min-h-[200px]">
                            <div className="card-body items-center justify-center text-center p-4 lg:p-8">
                                <div className="mb-3 lg:mb-4 p-3 lg:p-6 rounded-full bg-base-300">
                                    <svg className="w-10 h-10 lg:w-16 lg:h-16 text-primary" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                        <path d="M9 3v2m6-2v2M9 19v2m6-2v2M5 9H3m2 6H3m18-6h-2m2 6h-2M7 19h10a2 2 0 002-2V7a2 2 0 00-2-2H7a2 2 0 00-2 2v10a2 2 0 002 2zM9 9h6v6H9V9z" />
                                    </svg>
                                </div>
                                <h3 className="text-lg lg:text-2xl font-bold text-base-content mb-1 lg:mb-2">Ready to Debug</h3>
                                <p className="text-xs lg:text-base text-base-content/60 max-w-md">
                                    Enter your code in the editor and click "Debug Code" to analyze and fix issues automatically
                                </p>
                                <div className="mt-3 lg:mt-6 space-y-1.5 lg:space-y-2">
                                    {['Automatic bug detection', 'Smart code fixes', 'Validation & testing'].map(feature => (
                                        <div key={feature} className="flex items-center gap-2 text-xs lg:text-sm text-base-content/50">
