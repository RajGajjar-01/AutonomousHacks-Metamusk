    ...
    <div className="card bg-base-100 shadow-lg border border-base-200">
      <div className="card-body">
        <div className="flex items-center gap-2 mb-6">
          ...
          <h3 className="card-title text-base font-bold uppercase tracking-wider text-base-content/70">
            Processing Pipeline
          </h3>
        </div>
          ...
          {agents.map(agent => {
            ...
            return (
              <li
                ...
                className={stepClass}
                data-content={
                  agentStatus === 'complete' ? '✓' : agentStatus === 'working' ? '●' : ''
                }
              >
                    ...
                    {agentStatus === 'working' && status?.message ? (
                      ...
                      <span className="font-medium">
                        ...
                        {agent.name === 'Validator' &&
                          `${((result.confidence_score || 0) * 100).toFixed(0)}% conf.`}
                      </span>
                    ) : (
                      agent.description
                    )}
